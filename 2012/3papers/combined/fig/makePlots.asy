...
add(Legend(lineLength=3mm, NW), point(NW), Fill(white));
...
NewPad("", "$\si_{\rm tot}\ung{mb}$",xTicks = NoTicks(), yTicks=RightTicks(Step=5, step=1));
...
limits((0, 90), (8, 110), Crop);
//AttachLegend("total cross section");

//--------------------
NewRow();

NewPad("", "$\si_{\rm inel}\ung{mb}$", xTicks = NoTicks(), yTicks=RightTicks(Step=5, step=1));
real m = 73.3225;
draw((0.5, m)--(4.5, m), black+dense_dotted);
...
NewPad("", "$\si_{\rm el}\ung{mb}$", xTicks=NoTicks(), yTicks=RightTicks(Step=5, step=1));
real m = 25.175;
draw((0.5, m)--(4.5, m), black+dense_dotted);

...
In the previous TOTEM papers \cite{epl96,P1} the differential cross-section for elastic proton-proton scattering was presented as a function of the four-momentum transfer squared $t$. By extrapolation to $t=0$, the cross-section $\d\sigma_{\rm el}/\d t|_{0}$ at the optical point was determined. Integrating the differential distribution yields the integrated elastic cross-section $\sigma_{\rm el}$. Applying the optical theorem, the total and the inelastic proton-proton cross-sections were derived with a slight dependence on the $\rho$ parameter (the ratio of the real to the imaginary part of the forward hadronic scattering amplitude):
\begin{equation}
\label{eq:m el}
...
\end{equation}
The results for the low-luminosity measurement \cite{epl96} and the higher-luminosity measurement \cite{P1} are summarized in Tab.~\ref{tab:cs}. These cross-sections are exclusively based on the measurement of the elastic scattering cross-section.

However, taking advantage of the two triggerable forward charged-particle detectors, the inelastic cross-section was also directly measured by the inelastic rate \cite{P2} using the same data set as in \cite{P1}. A small Monte-Carlo correction ($\approx 3\%$) had to be applied to account for the non-visible events in the very forward cone $|\eta| > 6.5$ mainly due to single diffraction.
...
\item independent cross-sections
\item $\rho$ constraints
\item luminosity independent cross-section ratios
\end{itemize}
...
For completeness, some of the results will be compared to those of the low-luminosity run from June 2011 \cite{epl96}, where only the elastic part of the analysis could be performed.
...
One can use the luminosity-independent ratio $\sigma_{\rm el} / \sigma_{\rm inel}$ from the previous section to calculate the luminosity-independent $\sigma_{\rm inel} = (72.9 \pm 1.5)\un{mb}$ and $\sigma_{\rm el} = (25.1 \pm 1.1)\un{mb}$. All these cross-section values and their uncertainty compositions are summarized in Tab.~\ref{tab:cs}.
...
As shown above, the cross-sections can be obtained directly from the elastic scattering measurement, but also in combination with the inelastic rate eliminating  the dependence on either the luminosity or $\rho$. The total, elastic and inelastic cross-sections together with their individual uncertainties are summarized in Tab.~\ref{tab:cs} and also shown for comparison in Fig.~\ref{fig:cs}. All results are in excellent agreement with each other. In particular, the inelastic cross-section arising from two completely independent measurements with the Roman Pot detectors in the very forward region close to the beams and with the charged-particle telescopes more central agree remarkably well, giving confidence in the trigger, acceptance and analysis.

The energy dependence of the proton-proton cross-sections is shown in Fig.~\ref{fig:cs}. The best fit of $\sigma_{\rm tot}(s)$ by the COMPETE Collaboration \cite{compete} (before the TOTEM result) describes the energy dependence well even up to the highest energies. The energy dependence of the elastic cross-section is fitted by TOTEM.

\begin{figure*}
\begin{center}
\includegraphics{fig/sigma_tot_el_inel_cmp_big.pdf}
%\vskip-5mm
\caption{Left: the dependences of total (red), inelastic (blue) and elastic (green) cross-sections on the scattering energy $\sqrt s$. The continuous black lines (lower for $\rm pp$, upper for $\rm \bar pp$) represent the best fits of the total cross-section data by the COMPETE collaboration \cite{compete}. The dashed line results from a fit of the elastic scattering data. The dash-dotted curves correspond to the inelastic cross-section and were obtained as the difference between the continuos and dashed fits.\hfil\break
...
\vskip-5mm
\caption{(Color online) The ratio of the elastic to total cross-section as a function of the scattering energy $\sqrt s$. TODO: fit ??, REMOVE?}
\label{fig:sigma rat}
\end{figure}

...
\eqref{m_i - \hat m_i > n\, \si(m_i)\ ,}{al outlier cond}
where $\hat m_i = \vec d_i \cdot (\vec a^n z_i + \vec b^n)$ is the track interpolation and $\si(m_i)$ is the corresponding measurement error. $n$ is a parameter of the algorithm.
\itm Remove all hits from RPs where there are less hits per projection than a given limit $L$.
\itm If some points have been removed, go back to step 2. Stop otherwise.
\eitm

The event selection starts with a $\ch^2$ cut, that is all events with track fits the $\ch^2/\hbox{n.d.f.}$ of which exceeds a certain limit are discarded. Then, there are few optional (they can be switched on or off) boolean checks:
\> remove events with signal in a top and a bottom pot simultaneously,
\> require signal in both units,
\> require signal in the overlap between horizontal and vertical pots and
\> if there is signal in the overlap, require signal in at least three pots.
...
The basic elements in the track-based alignment are the detectors (sensors). Only these provide measurements and only these can be aligned. On the other side, the sensors are inserted (per groups) in RP and thus, they share (per groups) common misalignments. One could, therefore, try to extract these common misalignments and call them \em{RP alignments}. However, this should be still understood in the sense of common shifts and rotations of the sensors in a RP. A full alignment of RPs can to be obtained with track-based alignment.

\> the algorithm
\> why slopes not extracted
\> why errors not used

\subsection[al exp res]{Experimental results}

There are 3 sources: optical metrology, beam-tests in H8 and LHC data.

\ssubsection{Optical metrology}
...
\ssubsection{LHC runs}

\> Standard processing of the LHC data.
\>> minimumHitsPerProjectionPerRP=4
\>> process.RPStraightTrackAligner.removeImpossible = True
\>> requireBothUnits="True"
\>> requireOverlap="False"
\>> requireAtLeast3PotsInOverlap="True"
\>> constraintsType="final"; constraintsLabel="fin"
\>> iterations=5;
\>> optimize=("s" "s" "sr")
\>> maxResidualToSigma=(100 10 10 3);
\>> chiSqPerNdfCut=(5000 50 50 5)

\> LHC runs (maybe a table?)
\>> typical number of tracks used
\>> typical angular distributions
\>> list of data-takings


\ssubsection{Comparisons}
...
\Fg{al comp det per unit weak}: all runs possible (even without horizontal RPs), start at "standard" results, 5 iterations then, fix-ext

\fig{fig/pdf/al_comp_det_per_unit_weak.pdf}{al comp det per unit weak}{Alignment comparison with fixed-detectors constraints (rotz: 1200, 1201). Shows full rotation about z. October 24 (black), 26 (red), 29-30 (blue), sector 45}

(C)  LHC results per unit (choose one typical) on detector level

\fig{fig/pdf/al_comp_det_per_unit.pdf}{al comp det per unit}{Alignment comparison with final constraints. Unit 56 near. Data from 24 Aug to 29-30 Oct.

...
Selection of elastic events. What we know
\> angles the same left and right
\> correlation between angle and position
\> arrive at x ~ 0

Three cuts (for each diagonal)
\bitm
\itm $\th_x$ equal left and right.
\eqref{\hbox{distance from line } \De x^{56} = (-1 + a) \De x^{45} + b \hbox{ smaller than } 0.08\un{mm}\ ,}{al el cut 1}
where $a$ and $b$ are small corrections \TODO{determined in iterations}
In this section, $\De q \equiv q_F - q_N$

\itm $\De y$ proportional to $y$
\eqref{\hbox{distance from line } \De y = a y_N + b \hbox{ smaller than } 0.045\un{mm}\ ,}{al el cut 2}

\itm $x$ close to 0
\eqref{\hbox{distance from line } x = a y + b \hbox{ smaller than } 0.045\un{mm}\ ,}{al el cut 3}
\eitm

\fig{fig/pdf/al_el_selection.pdf}{al el selection}{Selection of elastic events. The black dots show events before the cut, colorful dots after. The red dots belong to 45 bottom -- 56 top diagonal, the blue to the other one. The lightgreen area represents the cut requirement. \TODO{data set, data reduction ?}}
...
\htab{al el y full}{y distributions, full fits}{
...
\htab{al el y gauss}{y distributions, guass region fits}{
\omit&\multispan4\bhrulefill\cr
\omit&\strut\hbox{45 near}&\hbox{45 far}&\hbox{56 near}&\hbox{56 far}\cr\bln
...
\htab{al el y shift}{y distributions, shift tests. The ucertainty can be estimated as a half of the bin size. That is $25\un{\mu m}$ for 26 and 29-30 Oct and $50\un{\mu m}$ for the others.}{
\omit&\multispan4\bhrulefill\cr
\omit&\strut\hbox{45 near}&\hbox{45 far}&\hbox{56 near}&\hbox{56 far}\cr\bln

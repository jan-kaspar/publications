...
\> The last option is called \em{final} since it has been agreed for the final alignment analysis. \Fg{al eig rho,al eig theta} suggest that, in standard conditions ($\De\rh_i = \O{5\un{mrad}}$, $\si(\vec a)=\O{0.1\un{mrad}}$), it is not possible to determine the rotation between U and V detectors and between near and far unit (following the weak mode \Eq{cnst rotz lt2} and the large $z$ difference between the units). Thus one needs to constrain these modes too. \TODO{We trust the vertical position of the beam from the touching excercises, we leave the horizontals out}. For every unit
\eqref{\eqnarray{
...
The final constraints \TODO{overconstrained}. So far we have discussed only the case where dim E = dim C, now also constraining also weak modes (dim E < dim C) and its danger when some of the weak modes are not that weak.
...
\fig{fig/pdf/al_syst_err_pitch.pdf}{al syst err pitch}{it3 - precise3, near top RP, every plane by different color. For rotz similar behavior in scale -0.5 to +0.5 mrad}

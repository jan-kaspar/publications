...
All our simulations have been done with the $220\un{m}$ station is sector 56. The proton tracks have been simulated from $z=217\un{m}$ (the point corresponding to $z=0$ in \Eq{al local track}). We have tried a number of track distributions, but the impact on the alignment results was small, as it should. For the figures in this section, we have used a track distribution which resembles the one observed in the LHC data -- both track slopes $a_x$ and $a_y$ generated according to a normal distribution with zero mean and variance $\si_a^2$, the intercepts $b_x$ and $b_y$ according to normal distributions with zero means and variances $\si_x = 6\un{mm}$ and $\si_y= 8\un{mm}$.

...
We have applied fixed-detector constraints. The fixed planes for read-out shifts were 0, 1, 8 and 9 (for every RP). For rotations about $z$: 0 and 1.

\tab{al H8 stat}{H8 statistics}{\bln
...
\vskip\baselineskip
An example of the internal alignment comparison is shown in \Fg{al comp det per pot}. For the other RPs, the comparison is yet worse.

\TODO{chose 2 or 3 pots that would show all things worth discussing - compatible and non-compatible results, possible linearly progressive rotation, etc.}

\fig{fig/pdf/al_comp_det_per_pot.pdf}{al comp det per pot}{Internal alignment comparison. All LHC points are overlapping (one can see only the top orange) and hence only few data-sets are shown.}
...
\> $U$-$V$ rotation impossible to determine, not even showing. Is there any hint from the optical metrology?
...
\TODO{Shift comparison is misleading because of different geometries!}

\fig{fig/pdf/al_comp_det_per_unit.pdf}{al comp det per unit}{Alignment comparison with final constraints. Unit 56 near. Every point corresponds to a data-taking from \Tb{al lhc datasets} (overlap=f). Dashed lines.}

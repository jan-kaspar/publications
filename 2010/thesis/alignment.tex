...
\fig[15cm]{fig/pdf/al_eigenvalues_rho.pdf}{al eig rho}{The dependence of $\mat S$ matrix eigenvalues on the RMS of $\rh$. Theta 10 mrad, geometry 2.7x3.3, overlap=f}
\TODO{note that singular modes have actually non-zero eigenvalues, choice of singular limit}
...
So far, we have assumed that tracks have a slope $\vec a^n$ and an intercept $\vec b^n$. However, the proton tracks at LHC are very parallel, making the slopes be distributed around 0 with spread of about $10^{-4}\un{rad}$. Let's look back to \Eq{cnst rotz 2} and check what happens when $\vec a^n\to 0$. We first expand the rotation matrix keeping just the first Taylor terms:
\eqref{\pmatrix{1 & -\De\rh_i\cr \De\rh_i & 1\cr} (\vec b^n - \vec c_i - \De\vec c_i) = {\vec a'}^n z_i + {\vec b'}^n - \vec c_i - \De\vec c'_i\ .}{cnst rotz lt1}
Comparing the $n$-dependent quantities one obtains:
\eqref{\pmatrix{1 & -\De\rh_i\cr \De\rh_i & 1\cr} \vec b^n = {\vec a'}^n z_i + {\vec b'}^n \ .}{cnst rotz lt2}
This condition has two solutions. First, $\De\rh_i \equiv \hbox{const.}$, which is equivalent to the solution of \Eq{cnst rotz 3}. Second solution
\eqref{\De\rh_i = \al z_i\ ,}{cnst rotz lt3}
where $\al$ is a proportionality constant. Then, ${\vec b'}^n = \vec b^n$ and ${\vec a'}^n = \al\, (-b^n_y,\ b^n_x)^\T$. In analogy to the arguments above \Eq{cnst rotz 7}, one can conclude that
\eqref{
\pmatrix{
\De\rh_i \cr
\De s_i
...
is a weak mode. It would be a true singular mode if we did not use the Taylor approximation of the rotation matrix. Or if all $\rh_i$'s would be zero. But as show in \Fg{al eig theta}, this weak mode is numerically indistinguishable from real singular modes.

It would be possible to draw a similar line of arguments for the case where $\vec b^n\to 0$. Then, $\De\rh_i \propto 1/z_i$ would emerge as a weak mode. But keeping in mind the application to LHC proton tracks, this turns out to be very unrealistic scenario.

The treatment above started with \Eq{cnst rotz 2}, which assumes that the read-out directions can have all values. In section \Sc{sing modes groups} we have derived that if there are just two read-out direction groups, the number of singular modes for rotations doubles. By similar arguments one find out that this is the case for the weak mode \Eq{cnst rotz lt4} too. There would be one such a mode for every group, just like in \Eq{cnst rotz g2}.

Eventually, let's remark that vanishing track angles does not introduce new weak modes for shifts in read-out nor in $z$ direction. The reason is simple, their $\ga$ factors (see \Tb{alignment quantities}) do not involve both track parameters (slope and intercept). Thus it is not possible that a misalignment would ''convert'' one to the other (in the sense of creating slope $\vec a'$ from intercept $\vec b$ in \Eq{cnst rotz lt2}).
...
\hbox{shift in }z					& 0 & 0\cr\ln
\hbox{rotation around}z				& 1 & 2\cr\bln
}

\fig[15cm]{fig/pdf/al_eigenvalues_theta.pdf}{al eig theta}{The dependence of $\mat S$ matrix eigenvalues on the RMS of $\th$. extfit=t means not-biased gammas. What is the misalignment? Show line at 0.1mrad as the typical value.}
...
\> \em{homogeneous}
\> \em{fixed detectors}
\> \em{final} (used for the final analysis)
...
\ssubsection{Systematical errors}
...
\> no systematics under indeal circumstances

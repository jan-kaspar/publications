...
\fig{fig/pdf/al_sing_modes_shr.pdf}{al sing modes shr}{The read-out-shift singular modes within a sensor package (of the nominal RP geometry). Odd sensors (the left most sensor has number 1) measure the $V$ projection, even the $U$ projection. The dots show the values of the singular mode for each sensor (in red for $V$ sensors, in blue for the $U$ ones).}
...
\fig{fig/pdf/al_sing_modes_shz.pdf}{al sing modes shz}{The $z$-shift singular modes within a sensor package. Left: the nominal RP geometry -- odd sensors (the left most sensor has number 1) measure the $V$ projection (red), even the $U$ projection (blue). The dots demonstrate the singular modes \Eq{al sm shz sol 2g}. Right: a hypothetical situation with more read-out groups, the dots represent the singular modes \Eq{al sm shz sol 3g}.}
...
\fig{fig/pdf/al_sing_modes_rotz.pdf}{al sing modes rotz}{The rotation singular modes within a sensor package. The top panels show the rotation values of the singular modes, the bottom the accompanying read-out-direction shift. Left: the nominal RP geometry -- odd sensors (the left most sensor has number 1) measure the $V$ projection (red), even the $U$ projection (blue). Right: a hypothetical situation where the 5th and 6th sensor have non-standard read-out directions (thereby increasing the number of read-out groups above 2).}

To illustrate the structure of the singular modes we have made a number of MC studies. We have considered a series of geometries, starting with the nominal one and adding rotation misalignments to all sensors. These mis-rotations have been randomly generated according to a normal distribution with a variable variance. Going from zero variance to positive values represents a smooth transition from a geometry with two read-out groups to geometries with more groups. This transition is shown in \Fg{al eig rho}. There, the results are presented in terms of \em{normalized eigenvalues}
\eqref{\la_{\rm N} = \hbox{eigenvalue of }\tilde\mat S/N_{\rm events}\ ,}{al S norm eig val}
which, in a good approximation, shall be independent of the number of events $N_{\rm events}$.

Any numerical calculation (we have used ROOT matrix libraries \bref{root matrix} with double precision) has a limited precision, thus even for true singular modes a small but non-zero eigenvalue is found. Therefore they look like weak modes. For any analysis one has to set a (non-zero) \em{singular limit} to distinguish between regular and very weak/singular modes. \TODO{value}

\fig{fig/pdf/al_eigenvalues_rho.pdf}{al eig rho}{The spectrum of normalized eigenvalues of the $\tilde\mat S$ matrix ($\la_{\rm N}$) as a function of geometry. The geometries considered have been created from the nominal one by applying rotation misalignment to each sensor (within one station). The misalignments have been randomly generated according to a normal distribution with zero mean and variance $\si^2(\De\rh)$ -- the quantity on the horizontal axis. The case $\si(\De\rh) = 0$ corresponds to the nominal geometry, which contains two read-out groups. Increasing $\si(\De\rh)$ then represents a smooth transition to configurations with three and more read-out groups.\hfil\break
The colors correspond to the eigenvalue order. Therefore, when two curves representing two alignment modes cross, they exchange their colors (e.g. the mode (4.47) in the bottom plot).
...
The LHC proton tracks are very parallel (see e.g. \Tb{al lhc datasets}), in other words the distribution of the track slopes $\vec a^n$ is sharply peaked about a mean value $\bar\vec a$:
\eqref{\vec a^n = \bar\vec a + \de\vec a^n,\qquad \si(\de\vec a^n) = \O{10^{-4}\un{rad}}\ .}{al sm a dist}
Let us reexamine the condition for rotation singular modes \Eq{al sm rotz con} when the spread $\de\vec a^n\to 0$:
\eqref{
...
$\be_g$ (equivalent of $\De\rh_g$ in \Eq{al sm rotz sol}) represents a global rotation, $\al_g$ leads to a linearly-progressive rotation. \TODO{ref to figure}, for a summary see \Tb{al sing mode overview}.

Unlike the geometrical singular modes, the modes \Eq{al sm rotz sol la} are singular for $\mat S$ and $\bar\mat S$ only, not for $\tilde\mat S$. The reason is that the transition \Eq{al tau linearization} from true to estimated track parameters brings in an error due to a finite resolution. Thus even if the true tracks are perfectly parallel, the reconstructed ones are not. Consequently the modes \Eq{al sm rotz sol la} correspond only to small, but non-zero eigenvalues of $\tilde\mat S$.

This effect is illustrated in the right panel of \Fg{al eig theta}. For true-track angular spread $\si(\de\vec a)$ below the angular resolution of a station (about few microradians), the eigenvalues of modes \Eq{al sm rotz sol la} do not reduce further as for $\mat S$ in the left panel.
...
\fig{fig/pdf/al_eigenvalues_theta.pdf}{al eig theta}{Spectra of normalized eigenvalues ($\la_{\rm N}$) of the $\mat S$ (left) and $\tilde\mat S$ (right) matrices plotted as a function of track-angle spread $\si(\de a)$ (same values used for $x$ and $y$ projections). The simulations have been done for a RP station of the nominal geometry (two read-out groups). For more groups there would be just one line instead of the labeled red and blue curves. The vertical dotted line marks the typical slope spread in the LHC data.
\TODO{parallel to non-parallel}
\TODO{nominal geometry - two groups}
% shr\_rotz=0. 2.7x33, overlap=f
}

\> Eventually, let's remark that parallel tracks do not introduce new weak modes for shifts in read-out nor in $z$ direction. The reason is simple, their $\ga$ factors (see \Tb{al alignment quantities}) do not involve both track parameters (slope and intercept). Thus it is not possible that a misalignment would ''convert'' one to the other (in the sense of creating slope $\vec a'$ from intercept $\vec b$ in \Eq{cnst rotz lt2}). \TODO{For z shifts, the number would indeed increase, with no slope spread, the z shifts can not be determined at all}.

\> final ref to \Tb{al sing mode overview}??

\htab{al sing mode overview}{Overview of singular and weak modes. The bold numbers give the number of singular modes that are listed afterwards. The ''glob.'' abbreviation stands for ''global'' which is used to refer to a mode with constant coefficients for every detector. The ''l.p.'' stands for ''linearly-progressive'' which means a mode the coefficients of which are proportional to $z_i$. In that sense X and Y shearings can be seen as linearly-progressive X and Y shifts. Note that the additional modes for the parallel-track case $\si(\de\vec a) = 0$ are weak only.}{

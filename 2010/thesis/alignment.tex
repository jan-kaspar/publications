...
\fig{fig/pdf/al_prof_simu.pdf}{al prof simu}{A Geant4 simulation of hit distributions at the RPs of the 56-220m near unit. The geen points represent elastic scattering (Elegent, PPP3), blue SD (Pythia) and red DPE (Phojet). All processes simulated at $\sqrt s = 3.5\un{GeV}$ and with nominal optics with $\be^* = 2.5\un{m}$. The black lines are the countours of the sensors.}

\fig{fig/pdf/al_prof_hits.pdf}{al prof hits}{Typical hit distributions at near stations (scoring planes at $\pm 217m$). Data from 21 Sep. The far stations look qualitatively the same.
% analysis with vsym2 geometry
}

\fig{fig/pdf/al_prof_fits.pdf}{al prof fits}{Details for the fit in 56 near unit. (data 21 Sep). The fit lines have the same color code as in \Fg{al prof hits}. The green points excluded from the fit.}

\TODO{Elastic peak tilts}

\TODO{Diffracive hits have banana shape, cannot be used}
...
Furthermore, the optics is such that the phase advance $\phi$ (see Hill's equation \Eq{hill eq}) at the $220\un{m}$ stations is close to $\pi/2$ in $x$ or $0$ in $y$ \TODO{reference}. This means that $L_x(220)$ almost vanishes, but $L_x'(220)$ is large and vice versa for the vertical projection. All these considerations lead to the following three properties of elastic events:
\eqref{\eqnarray{
...
\TODO{a, b: determined in iterations}
...
\em{Cut 3}. $x$ close to zero. As we saw in \Sc{al prof}, there is a sort of XY coupling in the optics, which makes the elastic distributions to tilt. Hence instead of expecting the elastic hits literally close to $x\approx 0$, we should require them being close to a line $x = ay + b$, where $a$ corresponds to the tilt and $b$ is a result of detector misalignments. The third cut reads:
\eqref{\hbox{distance from line } x = a y + b \hbox{ smaller than } 0.4\un{mm}\ .}{al el cut 3}

The cut parameters (a's, b's and thresholds) were determined empirically in several iterations. First we used the ideal values from \Eq{al el prop} and very liberal thresholds. The obtained sample was used to fit the relevant distributions and get corrections to the cut parameters. An illustration of the cuts can be found in \Fg{al el selection}.

\fig{fig/pdf/al_el_selection.pdf}{al el selection}{Selection of elastic events. The black dots show events before the cut, colorful dots after. The red dots belong to 45 bottom -- 56 top diagonal, the blue to the other one. The lightgreen area represents the cut requirement. Data from 21 Sep (only 2000 events shown).}



\em{Method 1}. vertical fit in YX plots for the x position of the beam (for every unit)

\fig{fig/pdf/al_el_plots_yx.pdf}{al el plots yx}{Illustration of the alignment method 1. The red line fits the red points only, similarly for the blue one. The green line represents a global fit. 45 far}

{\SmallerFonts
\htab{al el yx}{y vs. x fits, method 1. Slope $a$ in $\rm mrad$, intercept $b$ in $\rm\mu m$.}{
...
\em{Method 2a}. fit of vertical hit distributions for vertical alignment wrt. beam (for every unit), Gauss fit throught all data

\fig{fig/pdf/al_el_plots_y_full.pdf}{al el plots y full}{Illustration of the alignment method 2a. The violet curve shows a Gaussian fit. 56 far}

\htab{al el y full}{y distributions, full fits, values in $\rm \mu m$}{
...
\em{Method 2b}. As 2a, but fit through the Gaussian region only

\fig{fig/pdf/al_el_plots_y_gauss.pdf}{al el plots y gauss}{Illustration of the alignment method 2b. The violet curve shows a Gaussian fit. 56 far}

\htab{al el y gauss}{y distributions, guass region fits, values in $\rm \mu m$}{
\omit&\multispan4\bhrulefill\cr
\omit&\strut\hbox{45 near}&\hbox{45 far}&\hbox{56 near}&\hbox{56 far}\cr\bln
...
Comparison to method 2a: the worst case is $30\un{\mu m}$, generally much better than the error estimate.


\em{Method 2c}. A Kolmogorov-like shift test.

\fig{fig/pdf/al_el_plots_y_shift.pdf}{al el plots y shift}{Illustration of the alignment method 2c. The violet curve shows a Gaussian fit through the data in the fit region (delimited by the vertical dotted lines). The selected data points are marked in green. There are no data and no fit in the left plot. 56 far}

\htab{al el y shift}{y distributions, shift tests,  values in $\rm \mu m$. The ucertainty can be estimated as a half of the bin size. That is $25\un{\mu m}$ for 26 and 29-30 Oct and $50\un{\mu m}$ for the others.}{
\omit&\multispan4\bhrulefill\cr
\omit&\strut\hbox{45 near}&\hbox{45 far}&\hbox{56 near}&\hbox{56 far}\cr\bln
...
Comparison to method 2a: std. deviation of the differences $30\un{mu m}$, generally well within the estimated error.



\em{Method 3}. fit of dy vs. y for far wrt. near vertical alignment (for every arm)

\fig{fig/pdf/al_el_plots_dyy.pdf}{al el plots dyy}{Illustration of the alignment method 3. The red line fits the red points only, similarly for the blue one. The green line represents a global fit. sector 45}

\bgroup
\def\ln{\multispan3\hrulefill&&\multispan2\hrulefill\cr}
\def\bln{\multispan3\bhrulefill&&\multispan2\bhrulefill\cr}
\htab{al el dyy}{dy vs y fits}{
...
\em{Method 4}. fit of y56 vs y45 for left wrt. right vertical alignment (for far and near units)

\fig{fig/pdf/al_el_plots_ylyr.pdf}{al el plots ylyr}{Illustration of the alignment method 4. The red line fits the red points only, similarly for the blue one. The green line represents a global fit. far units}

\bgroup
\def\ln{\multispan3\hrulefill&&\multispan2\hrulefill\cr}
\def\bln{\multispan3\bhrulefill&&\multispan2\bhrulefill\cr}
\htab{al el ylyr}{y56 vs. y45 fits}{
\omit&\multispan2\bhrulefill&&\multispan2\bhrulefill\cr
...
Methods 1 and 3, 4: all the fits are the same - the same misalignment of top and bottom pots. In other words, the TB alignment worked well!

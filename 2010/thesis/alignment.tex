...
\bitm
\itm First the RPs shall be approached to the desired position as precisely as possible, then their position with respect to the beam shall be verified  with dedicated devices such as the beam position monitors (BPMs) and the linear voltage differential transformators (LVDTs), for details see \Sc{al collim}.
...
The nominal sensors' positions (see \Sc{ttm det}) are perpendicular to the $z$ axis, that is $\rh_x=\rh_y=0$. The order angular misalignments $\De\rh$ in summarized in \Tb{al exp misal}. Taking $5\un{mrad}$ as the order of these angles, one obtains
...
This form of \Eq{al effective measurement all} is convenient for an application of the linear least squares (LS) method (see e.g.~Sc.~6.6 in \bref{barlow}). The method gives the following prescription for the estimate of the parameter vector $\vec P$:

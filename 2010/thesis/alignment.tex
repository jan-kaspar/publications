...
\fig{fig/pdf/al_prof_simu.pdf}{al prof simu}{A Geant4 simulation of hit distributions at ?? (what processes?)}

\fig{fig/pdf/al_prof_hits.pdf}{al prof hits}{Typical hit distributions at near stations, where exactly ??. data from 21 Sep. The far stations look qualitatively the same.
...
4 alignment methods
\bitm
\itm vertical fit in YX plots for the x position of the beam (for every unit)
\itm fit of dy vs. y for far wrt. near vertical alignment (for every arm)
\itm fit of y56 vs y45 for left wrt. right vertical alignment (for far and near units)
\itm fit of vertical hit distributions for vertical alignment wrt. beam (for every unit)
\eitm

\fig{fig/pdf/al_el_plots.pdf}{al el plots}{Illustration of the elastic alignment methods. For methods 1 to 3 there are always three fits. The red line fits the red points only, similarly for the blue one. The green line represents a global fit. For the method 4, the violet curve shows a Gaussian fit.}

Methods 1 and 2: all the fits are the same - the same misalignment of top and bottom pots. In other words, the TB alignment worked well!

\TODO{Summary table, comparison of methods}

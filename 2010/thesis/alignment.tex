...
\hbox{shift in }z 		& ??					& \approx 1\un{mm} ??\cr\ln
...
First of all, let us remark that any error that might appear can be of few origins only: the \em{approximations} used to derive the alignment equations and \em{neglected effects} (e.g. multiple scattering) and \em{data corruption} (electronic noise, problems in the DAQ chain etc.). One can check the presence of problems of the latter type by dividing the sample into a number of subsamples. Were the results obtained from the subsamples not compatible with each other, these effects would be important (however, in \Sc{al exp res} we will see it is not the case). Now, let us review the approximations we have made.
...
As the last comment related to the iterations, let us mention the typical number of steps. Both in MC tests (\Sc{al mc tests}) and LHC data analysis (\Sc{al exp res}), two iterations have turned out to be sufficient. That is, the correction in the third one has already been negligible.
...
Two examples of such statistical studies can be found in \Fg{al stat fixDet} (with a set of fixed-detector constraints) and \Fg{al stat final} (with the final constraints).
...
The values of the selection algorithms parameters will be given for each analysis in \Sc{al exp res}. Let us only remark that some of these parameters will be changed during iterations. For example the parameter of the outlier cut \Eq{al outlier cond} or the one of the $\ch^2$ cut will be made increasingly strict, as we will approach the expected alignment corrections (see the \rhs~column in \Tb{al iter}) decrease.


\subsection[al rp fac]{Alignment of detector packages}

The basic structures in the track-based alignment are the detectors (sensors). Only these provide measurements and only these can be aligned. On the other side, the sensors are inserted (per groups) in RPs and thus, they share (per groups) common misalignments. One could, therefore, try to extract these common misalignments and call them \em{DP alignments}. However, this should be still understood in the sense of common shifts and rotations of the sensors in a DP. In no way, the track-based alignment can determine the positions of inactive elements, like the thin window.

\fig{fig/pdf/al_rp_misalignment.pdf}{al rp misalignment}{A side view on a DP (for simplicity only 4 planes are drawn). The displaced detectors are drawn in black, their centers are marked with dots. The centers of non-displaced detectors are shown as gray dots. The blue arrows represent $\vec c_i$ vectors from \Eq{al rp de c}. The green arrow represents the DP shift $\vec s^{\rm DP}$.}

Any DP misalignment can be decomposed to the shift of its center $\vec s^{\rm DP}$ and the rotation about its center. The rotation can be represented by a matrix $\mat R^{\rm DP}$, an equivalent of the rotation matrix for sensors (cf.~\Eq{al global to local}). Let us define the center of a DP ($\vec c^{\rm DP}$) as the mean of its sensors' centers. The DP misalignments are naturally inherited by all sensors of this DP, but moreover, the rotation may give rise to additional shifts, see \Fg{al rp misalignment}. The center of $i$-th detector is
\eqref{\vec c'_i = \mat R^{\rm DP} (\vec c_i - \vec c^{\rm DP}) + \vec c^{\rm DP} + \vec s^{\rm DP}\ ,}{al rp c'}
where $\vec c$ would be its nominal center (without the DP misalignment). The only non-negligible component of $\vec c_i - \vec c^{\rm DP}$ vector is the $z$ one. Furthermore, since the expected mis-rotations are of the order \TODO{?? reference}, we can use the approximation \Eq{al rotation parameterization approximated} for the rotation matrix $\mat R^{\rm DP}$. Then, keeping only $x$ and $y$ components, one finds
\eqref{\De\vec c_i \equiv \vec c'_i - \vec c_i = \vec s^{\rm DP} + \pmatrix{\rh_y^{\rm DP}\cr \rh_x^{\rm DP}} z_i^{\rm eff}, \qquad z_i^{\rm eff} = c_{z_i} - c_z^{\rm DP}\ .}{al rp de c}
Since the sensors are placed in DPs in regular $z$ intervals, one can expect the shifts to be linearly dependent on the plane number. And indeed, this can be seen in experimental data, for example in \Fg{al comp det per unit}. The shifts in read-out direction (this is what is determined by track-based alignment) can be compactly written
\eqref{
\pmatrix{\vdots\cr \De s_i\cr \vdots} = \mat F \pmatrix{s_x^{\rm DP}\cr s_y^{\rm DP}\cr \rh_x^{\rm DP}\cr \rh_y^{\rm DP}\cr}, \qquad
\mat F = \pmatrix{
\vdots & \vdots & \vdots & \vdots\cr
d_{x_i} & d_{y_i} & d_{x_i}\,z_i^{\rm eff} & d_{y_i}\,z_i^{\rm eff} \cr
\vdots & \vdots & \vdots & \vdots\cr
}\ ,}{al rp fit matrix}
where $i$ lists all sensors in the given DP. The last equality can easily be combined with the Least Squares method in order to determine the DP misalignments from experimental data:
\eqref{\pmatrix{s_x^{\rm DP}\cr s_y^{\rm DP}\cr \rh_x^{\rm DP}\cr \rh_y^{\rm DP}\cr} = (\mat F^\T \mat V^{-1} \mat F)^{-1} \mat F^\T \mat V^{-1} \pmatrix{\vdots\cr \De s_i\cr \vdots}\ ,}{al rp fit}
...
As it has been said above, all sensors in a pot inherit its rotation misalignment. Since we can determine sensors' rotations about $z$, we can extract the DP rotation by simply taking the mean rotation
\eqref{\rh_z^{\rm DP} = {\sum_i {\rh_{z_i}\over \si^2_i} \over \sum_i {1\over\si_i^2}}\ .}{al rp rotz fit}
Again, $i$ lists all sensors in the given DP and $\si_i$ abbreviates the uncertainty of $\rh_{z_i}$.

All three DP rotations extracted from LHC data can be found in \Fg{al comp rp all rot}.
...
The position of the detectors within the RP assembly has been measured. There are 3 reference points on a detector and one point on the RP. For each detector a zoomed high resolution photo was taken and a relative position of the points 1 and 2 (see \Fg{al optical metrology}) and the RP reference point was measured. Theoretical values and results are summarized in \Tb{metrology theoretical}. The precision of this measurement is $\approx 10\un{\mu m}$.

\fig{fig/pdf/al_optical_metrology.pdf}{al optical metrology}{An illustration of the optical metrology measurement. \TODO{points on detector or hybrid?} \TODO{add mark photo}}

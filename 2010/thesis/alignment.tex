...
If we put the singular vectors of $\tilde\mat S$ into columns of an $\mat Q_{\rm s}$ matrix, we get $\tilde\mat S \mat Q_{\rm s} = 0$. The $\tilde\mat S$ matrix is also symmetric, thus one may write
\eqref{
	\mat Q_{\rm s}^\T (\tilde\vec T - \mat C\vec\La)
	= \mat Q_{\rm s}^\T \tilde\mat S
	= \mat Q_{\rm s}^\T \tilde\mat S^\T
	= (\tilde\mat S \mat Q_{\rm s})^\T = 0\ .
}{al T no sing cont}
...
\eqref{\hat\vec\ch = \vec\ch^0 + \mat Q_{\rm s} \vec\ch_{\rm s}\ ,}{al sol param}
where $\vec\ch^0$ is a particular solution and $\vec\ch_{\rm s}$ are the parameters in the nullspace of $\tilde\mat S$.
%\eqref{\vec\ch^0 = \mat Q_{\rm r} \mat L^{-1} \mat Q_{\rm r}^\T \vec T'}{al cnst chi 0}
Another requirement we want to imply on the set of constraints $\mat C$ is to determine the solution $\hat\vec\ch$ uniquely. By inserting the above parameterization to \Eq{al constraints}, one finds that the requirement is equivalent to asking $\mat C^\T \mat Q_{\rm s}$ to be regular, which is further equivalent to the previous requirement \Eq{al ETC reg}.

%\eqref{\vec\et = (\mat C^\T \mat E)^{-1} (\vec V - \vec C^\T \vec\ch^0)}{al cnst eta}
...
$$
The $\mat M$ matrix receives two contributions -- from weak and regular modes. To perform the inversion, one may use the recurrent relation:
$$(\mat A+\mat B)^{-1} = \mat A^{-1} - \mat A^{-1} \mat B \mat A^{-1} + \mat A^{-1} \mat B \mat A^{-1} \mat B \mat A^{-1} \ldots\ ,$$
which as a form of "power series" -- taking into account that $\mat A^{-1}$ contains the small eigenvalues of the weak modes. Because of that, it is sufficient to take first few terms only. Then, provided that $\mat Q_{\rm w}^\T \mat C$ is regular, it is straight-forward to find that the solution $\hat\vec\ch$ does not diverge when the weak eigenvalues vanish. In that limit, the solution gains the form of \Eq{al sol param}.
}.
...
Second, in the limit where the weak modes become singular (their eigenvalues tend to zero), the solution becomes equivalent to \Eq{al sol param}. Hence there is no difference between extremely weak and singular modes. This is important since computer programs treat singular modes as weak.

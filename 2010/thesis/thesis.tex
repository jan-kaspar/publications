...
(The hat is to emphasize it is an estimate.) However, the $\mat\Ga$ matrices (contained in $\mat A$ matrix) include track parameters which are unknown at the fit time. Instead, one can use track parameter estimates obtained via model where all misalignments are neglected. This is basically using $\mat \al^n$ as fit matrix for data $\vec \mu^n$. Let's denote gamma and A matrices obtained in this way as $\hat\mat\Ga$ and $\hat\mat A$:
\eqref{\vec\ta \rightarrow \hat\vec\ta: \quad \mat\Ga \rightarrow \hat\mat\Ga, \mat A \rightarrow \hat\mat A}{tau linearization}
 One can then use
...
\eqref{\mat S\, \hat\vec\ch = \vec T, \qquad 
\vec T = \pmatrix{
...
\eqref{\mat S \hat\vec\ch = \vec T - \mat C \vec\La \equiv \tilde\vec T(\vec\La)\ ,}{cnst aux1}
...
\eqref{\mat D \mat Q^{-1} \hat\vec\ch = (\mat Q^\T \mat Q)^{-1} \mat Q^T \tilde\vec T(\vec\La)}{cnst aux4}
...
\tilde\vec T(\vec\La)
\ .}{cnst aux5}
The part above the vertical line on the \lhs{} is identically equal to zero and thus so the \rhs{} must be. Considering that $\mat E^T \mat E$ is regular, one finds
...
For the moment, let's get back to \Eq{cnst aux1}. (As $\vec\La$ has been resolved from \Eq{cnst aux6}, $\tilde\vec T$ is known.) This equation has infinite number of solutions that can be written
...
Now it would nice to show that the actual misalignments can play the role of $\vec\ch^0$. Unfortunatelly, this is the case only for shifts in read-out direction. For the other misalignment classes it is violated because of the linearization step \Eq{tau linearization}. To demonstrate that we will assume this has not been done and only afterwards we will show where the problem appears. Let's use the $\vec\ch_{\rm a}$ symbol for the actual misalignments.
Combining \Eq{todo1,vec R,fit equation} one gets
\eqref{\vec T = \mat S \vec\ch_{\rm a}}{cnst form}
and since $\mat E^\T \mat S \equiv 0$, $\vec\La = 0$ and $\tilde\vec T = \vec T$.

\> $\vec\ch^0$ is the true solution for shifts only

\TODO{$\vec\La$ should be close to zero}

...
rGetObj(file, "vertex/reco global vs. smeared/vtx_x_rs_diff"); robj.vExec("Rebin", 4); draw(xscale(1e3), robj, blue, "$\De x^*$");
label("RMS = "+format("$%#.2f$", robj.rExec("GetRMS")*1e3)+"$\ \rm mm$", (-5, 1.8e4), blue);
rGetObj(file, "vertex/reco global vs. smeared/vtx_y_rs_diff"); robj.vExec("Rebin", 4); draw(xscale(1e3), robj, red, "$\De y^*$");

...
block b_o = rectangle("\strut optics tuning", (5.5, 1.5), white); draw(b_o);

block b_rd = bevel("\strut RP tracks", (0, 0), white); draw(b_rd);
block b_al = rectangle(Label("\strut alignment", white), (2.5, 0), heavyred); draw(b_al);
block b_rec = rectangle("\vbox{\hbox{\hskip5mm angular}\hbox{reconstruction}}", (5.5, 0), white); draw(b_rec);
block b_esel = rectangle("\strut event selection", (8.8, 0), white); draw(b_esel);
block b_bs = rectangle(Label("\strut background subtraction", white), (12.7, 0), heavyred); draw(b_bs);

block b_fva = rectangle(Label("\strut final vertical alignment", white), (7, -1.2), heavyred); draw(b_fva);

block b_acc = rectangle("\strut acceptance correction", (12.7, -1.2), white); draw(b_acc);

block b_unsm = rectangle(Label("\strut unsmearing", white), (12.7, -2.4), heavyred); draw(b_unsm);
block b_norm = rectangle("\strut normalization", (9.5, -2.4), white); draw(b_norm);
block b_dsdt = bevel("\strut $\d\si/\d t$", (5.5, -2.4), white); draw(b_dsdt);
...
add(new 
	void(picture pic, transform t) {
		...
		draw(pic, b_acc.bottom(t)--b_unsm.top(t), Arrow);
		draw(pic, b_unsm.left(t)--b_norm.right(t), Arrow);
		draw(pic, b_norm.left(t)--b_dsdt.right(t), Arrow);
		
		draw(pic, b_o.bottom(t)--b_rec.top(t), Arrows);
...
shipout("felm_scheme_slides",  bbox(1mm, Fill(white)));

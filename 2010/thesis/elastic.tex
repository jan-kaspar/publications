...
The optical theorem (see e.g.~Sec.~7.3 in \bref{peskin} or Sec.~5.1.3 in \bref{formanek QFT}) relates the total cross-section $\si_{\rm tot}$ to the amplitudes that preserve the spin states. From \Eq{el hel amp} we see that those are the amplitudes $\phi_1$ and $\phi_3$. Using the normalization from \bref{buttimore78} one can write:

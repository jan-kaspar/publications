...
The diffraction amplitude $F_{\rm D}(t)$ is formulated in the eikonal formalism (see e.g.~Ch.~6 in \bref{barone}):
\eqref{
	F_{\rm D}(t) = \i p \sqrt s \int\limits_0^B \d b\, b\, J_0(b \sqrt{-t})\, \Ga_{\rm D}^+(b)
}{el is F D}
The upper integration bound $B$ should theoretically tend to infinity. This is, however, unrealizable for computer programs. In our calculations we have found a value of $B = 50\un{GeV^{-1}}$ reasonable. The eikonal $\Ga_{\rm D}^+(b)$ represents the crossing-even component (thus identical for $\rm pp$ and $\rm\bar pp$ scatterings). The authors argue that for the $t$-range where the diffraction dominates, the crossing-odd component is negligible. The eikonal is described by a Fermi profile function:
\eqref{\Ga_{\rm D}^+(b) =
	g(s)\, \left[
...
The authors have shown (e.g.~\bref{islam03}) that this diffraction amplitude satisfies the general properties associated with diffraction, e.g., qualitative saturation of Froissart-Martin bound ($\si_{\rm tot} \sim (a_0 + a_1\log s)^2$) or asymptotic behavior of $\rh \equiv \Re F_{\rm D}(0) / \Im F_{\rm D}(0)$ ($\rh \sim \pi a_1 / (a_0 + a_1 \log s)$).

The underlying quantum-field-theory (\abb{QFT}) model suggests that core-core scattering (amplitude $F_{\rm H}$) it is mediated by the $\rm\om$ meson, acting as an elementary particle (not like a Regge pole in many other models):
\eqref{F_{\rm H}(t) = \mp {\cal H}(s)\, {s\, F_{\rm\om}^2(t) \over m_\om^2 - t}\ .}{el is F core}
Hereafter, the upper (lower) sign will correspond to $\rm pp$ ($\rm\bar pp$) scattering. The form factor $F_\om(t)$ was determined by requiring the core-core amplitude $F_{\rm H}(t)$ to fall off exponentially \bref{TODO}:
...
The valence-quark amplitude $F_{\rm Q}(t)$ accounts for high-$|t|$ scattering where the inner-most proton layers (the valence quarks in this model) interact. Besides the diffraction-absorption factor ${\cal H}(s)$, the amplitude includes a form factor ${\cal F}$ reflecting the quark confinement and an amplitude $F_{\rm qq}$ describing the basic interaction of two quarks:
\eqref{
	F_{\rm Q}(t) = {\cal H}(s) \, {\cal F}^2(q_\perp) \, F_{\rm qq}(t)\ ,\qquad
	%q_\perp = \sqrt{t\left( {t\over t_{\rm min}} - 1 \right)}
	q_\perp \simeq \sqrt{-t}\ .
}{el is F qq}
The basic idea of the form-factor construction is that the quarks have a certain spatial distribution within the quark bag. This distribution is described by an exponential $\exp(- m_0 r)$, where $r$ gives the distance from proton's center and $m_0$ is constant inversely proportional to the quark-bag size. At the end, the form factor is expressed as:
\eqref{
...
Regarding the basic quark-quark amplitude $F_{\rm qq}(t)$, the model comprises two variants. In the first one (we will refer to it as \abb{HP} variant) \TODO{ref \bref{islam05} }, the quarks interact via the hard pomeron (reggeized gluon ladders, as described by next-to-leading order \abb{BFKL}). In the model of Islam et al., the hard pomeron is approximated by a single pole (with position determined by $r_0$). The intercept of the pomeron is given by the parameter $\om$: $\al_{\rm BFKL} = 1 + \om$. The hard-pomeron amplitude reads:
\eqref{F_{\rm HP}(t) = {\i\, \tilde\ga_{\rm qq}\, s\, \left(s \e^{-\i {\pi\over 2}} \right)^\om \over {1\over r_0^2} - t} \ .}{el is F HP}

In the other variant (see \bref{islam09}, denoted \abb{LxG}), the interaction is mediated by the low-$x$ gluons that surround the valence quarks:
\eqref{F_{\rm LxG}(t) = {\i\, \tilde\ga_{\rm gg}\, s\, \left(s \e^{-\i {\pi\over 2}} \right)^\la \over \left(1 - {t\over m_c^2} \right)^{5\over 2}}\ .}{el is F HP}
The parameter $m_c$ is inversely proportional to the size of the gluon cloud around the valence quarks. $\la$ controls the low-$x$ gluon distribution function, which can be approximated by $x^{-\la}$. For details...


\vskip1cm\hrule\vskip1cm

% This comment is about placing the references in your description of our model. I suggest: a)  after the sentence "Their spatial distribution . . . from the nucleon's center", insert the reference : Islam, Luddy, Prokudin, Int. J. Mod. Phys., 2006, vol. A21, p. 1- 42 as ref. [15]. b) Then after the sentence "In the first one (we will refer to it as HP) . . . (gluon ladders in BFKL)", insert ref. Islam, luddy, Prokudin, Phys. Lett., 2005, vol. B605, p. 115 - 122 as ref. [16]. c) Change your reference [17] to Islam, Kaspar, Luddy, Deep-Elastic pp Scattering at LHC from Low-x Gluons, Mod. Phys. Lett., 2009, vol.A24, no. 7,p. 485 - 496, and insert it at the end of the sentence"In the second (LxG), . . . that surround the valence quarks".

19 parameters in total

diff (7): $R_0$, $R_1$, $a_0$, $a_1$, $\et_0$, $c_0$, $\si$
core (2): $\be$, $m_\om$
absorption (6): $\hat\ga_0$, $\hat\ga_1$, $\hat\si$, $\la_0$, $d_0$, $\al$
confinement (1): $m_0$
hard pomeron (3): $\tilde\ga_{\rm qq}$, $\om$, $r_0$
low-$x$ gluons (3): $\tilde\ga_{\rm gg}$, $\la$, $m_c$
...
The lower integration bound was set to $T = -400\un{GeV^2}$ in our programs. In order to increase the performance of our numerical calculations, we first sample ($10^4$ points) the eikonal $\Om_0(b)$ on the interval $\langle 0, B\rangle$. Then, we employ a linear interpolation in \Eq{el bsw F}.
...
This model is a \abb{QCD}-inspired, eikonal-formulated model. The scattering amplitude is given in terms of the Fourier-Bessel transform:
...
\right]}{el bh ch}
reflecting (in the same order) gluon-gluon, quark-gluon, quark-quark interactions and an odderon exchange. The odderon term is responsible for the difference between $\rm pp$ and $\rm\bar pp$ scattering at low energies and is negligible at higher (\abb{LHC}) energies. Each of the eikonal contributions is factorized into the energy dependence $\si(s)$ and the impact-parameter profile $W(b, \mu)$. The parameters $\mu$ describe the ``areas'' occupied by quarks or gluons in the nucleon. The impact-parameter profiles are parameterized as follows:
...
and calculated the integral in \Eq{el bh si gg} analytically (this yields a long expression that can be found in Eq.~(B5) in \bref{bh99}).

The quark-quark cross-section is simulated by a constant and falling component:
...
We will refer to this expression as the \WY{} or shortly \abb{WY} formula. The authors estimate the uncertainty of the \rhs{} to be of the order of $\al$.
...
where $\ga \doteq 0.577$ is the Euler's constant. This result will be referred to as the simplified \WY{} or shortly \abb{SWY} formula.
...
}. Moreover they identified Cahn's integration bound $T$ (see \Eq{el phase Cahn}) with the boundary of the physics region of momentum transfer $t_{\rm min}$, such that the integration in \Eq{el phase Cahn} is constrained to the physical region only (see also the discussion above \Eq{el T 1}). Their result reads (\KL{} or shortly \abb{KL} formula):
\eqref{\Ps_{\rm KL}(t) =
...
This phase will be referred to as the corrected \KL{} or shortly \abb{CKL} formula. We will discuss the meaning of the parameter $T$ later on (see the text around \Eq{el T 2}). For the time being we can identify $T = t_{\rm min}$ to recover the formula of \KL{} or let $T\to -\infty$ for Cahn's formula (for no form factor $G_{\rm eff}^2(t) \equiv 1$ one can simplify $I(t, t') = -2\pi / |t' - t|$).
...
The simplified \WaY{} formula, \Eq{el phase SWY}, brings yet another assumption -- constant slope $B$, which is undoubtedly ruled out by experimental data. No surprise that the prediction of the simplified formula deviates significantly from the non-simplified one for $|t| \gs 3\cdot 10^{-2}\un{GeV^2}$, see \Fg{el cic noff Psi,el cic diff Psi ff}. We may only wonder why the simplified formula is still used, see for instance Eq.~2-7 in \abb{ATLAS} \abb{ALFA} \abb{TDR} \bref{alfa}.

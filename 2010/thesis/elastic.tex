...
\em{Eikonal approximation} is ... (see e.g. Sc.~2.3 in \bref{barone} or Sc.? in \bref{formanek qm}). It sais
...
\eqref{\de(b) = ... \int_{-\infty}^{\infty} V(\vec b, z)\ \d z,}{el eik app}
where $V(x, y, z)$ is a potential. As the eikonal is linear in potential, it retains the property of additivity. The eikonal can also be related to the Born approximation of a scattering amplitude
\eqref{\de(b) = {1\over s} \int q\,\d q\,J_0(bq)\, F_{\rm Born}(t = -q^2)}{el eik app born}

With the Coulomb potential $V(r) = \pm ... \al/r$ one gets \TODO{ref}
\eqref{F^{\rm C}_{\rm Born} = \pm ... {\al\over t}}{el coul born}
From point-like to charge distributions of both target and projectile, it hopefully adds a factor of $F^2(t)$. Compare to \Eq{el dsdt em} (Born -- OPE). Take sign from eikonal formula (note that QED amp. has different signs too) and take FF from QED calculation. In that sense, it is not a Coulomb scattering anymore (magnetic influence is accounted for too), but we will keep the notation.
...
Bethe derived \bref{bethe58} by WKB approximation
\eqref{F^{C+H}(t) = F^C(t) e^{i \al \Ph(t)} + F^H(t)}{el FCH decomp C}
with the Coulomb-hadronic phase $\Phi$
\eqref{\Phi_{\rm Bethe}(t) = 2 \log {1.06\over a\sqrt{t}}}{el phase bethe}

No FF, $F_{\rm C}$ is just a symbol...
...
The same decomposition as \Eq{el FCH decomp C}, $F_{\rm C}$ only a symbol for ...

\eqref{\Phi_{\rm WY} = - \log {t\over t_{\rm min}} - \int\limits_{t_{\rm min}} {\d t'\over |t' - t|} \left( {F^H(t')\over F^H(t)} - 1 \right)}{el phase WY}
Note that Eq.(23) in \bref{wy68} is likely to contain a missprint: the sign of the second term shall read minus.

If $F^H(t) \propto e^{Bt/2}$, then the phase would simplify (for low scattering angles and high energies) to
\eqref{\Phi_{\rm SWY} = - \left(\log {B|t|\over 2} + \ga \right) \ ,}{el phase SWY}
...
\eqref{\Ps_{\rm Cahn} = \lim\limits_{t_{\rm min} \to -\infty} \left[ \log {t\over t_{\rm min}} - \int\limits_{t_{\rm min}}^{0} {\d t'\over |t' - t|} \left ( {F^H(t')\over F^H(t)} - 1 \right) \right]}{el phase Cahn}

\TODO{Check signs}

Simplification of $G_{\rm eff} = F_D$ leads to
\eqref{\nu_{\rm Cahn} = - {4t\over \La^2} \log {\La^2\over -4t} + {2t\over \La^2}}{el nu Cahn}
Independent of $s$

\KaL{} \bref{kl94}: generalization for form factors

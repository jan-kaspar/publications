...
The authors picture the proton in an effective quantum-field-theory (\abb{QFT}) model. They proposed a soliton solution with a mass comparable to the one of proton. This soliton divides the proton into two distinct layers: an outer cloud of quark-antiquark condensate and an inner core of topological charge. Later on, a third, the inner-most layer has been added -- a valence-quark bag. These three layers give rise to three mechanisms contributing to elastic proton-proton scattering. Each of them drives the amplitude in a range of momentum-transfer $t$.

% 1. This concerns the statement: "The corresponding scattering amplitude is formulated in the eikonal approach (see e.g. Ch 6 in [10]), with a Gaussian-like profile function". The first part of the statement is correct, but not the second part: "with a Gaussian-like profile function". We chose an even Fermi-profile function [13] ( Eq.(3) ), not a Gaussian one. Furthermore, we pointed out that with lns-dependent parameters, we obtain a diffraction amplitude that satisfies the general properties associated with diffraction, e.g., qualitative saturation of Froissart-Martin bound for the total cross section, asymptotic behavior of rho(s) (Re F(s,0)/ImF(s,0)) etc.[14].
...
For the lowest $|t|$ values ($\ls 0.5\un{GeV^2}$ at \abb{LHC} energies) it is the diffraction scattering, originating from a glancing overlapping of the outer clouds. The corresponding scattering amplitude is formulated in the eikonal approach (see e.g.~Ch.~6 in \bref{barone}), with a Gaussian-like profile function. For higher $|t|$ values (up to $\approx 5\un{GeV^2}$ at \abb{LHC} energies) the leading mechanism is the core-core scattering. The \abb{QFT} model suggests that it is mediated by the $\rm\om$ meson, behaving as an elementary particle (not like a Regge pole in many other models). For even higher values of $|t|$, the scattering is probing the inner-most layer of valence quarks. Their spatial distribution is assumed to fall off exponentially as departing from the nucleon's center. There are two alternatives for the interaction of two valence quarks. In the first one (we will refer to it as \abb{HP}), the quarks interact via a hard pomeron (gluon ladders in \abb{BFKL}). In the second (\abb{LxG}), the interaction is mediated by the low-$x$ gluons that surround the valence quarks.

The model contains 19 parameters in the hard-pomeron version \abb{HP} or 20 in the low-$x$ gluon version \abb{LxG}.

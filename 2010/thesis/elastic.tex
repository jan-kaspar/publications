...
Amplitude (integration bound $B = 50\un{GeV^{-1}}$)
\eqref{F(t) = \i p\sqrt s \int\limits_0^B \d b\,b\, J_0(b \sqrt{-t}) \left( 1 - \e^{\Om_0(b)}\right)}{el bsw F}
Eikonal (integration bound $T = -400\un{GeV^2}$) pre-calculated -- $10^4$ samples on $(0, B)$, later linearly interpolated
\eqref{\Om_0(b) = {1\over 2} \int\limits_T^0 \d t\, J_0(b\sqrt{-t})\, \tilde\Om_0(t)}{el bsw Om}
$S_0$ in fact also fcn of $t$, the factor before the background term is problematic
\eqref{\tilde\Om_0(t) = S_0(s)\, F_0(t) + {1\over s} R_0(t)}{el bsw tilde Om}
...
Background Regge trajectories
\eqref{
	R(t) = R_{\rm a_2}(t) + R_{\rm\om}(t) + R_{\rm\rh}(t)\ ,\qquad
	R_j = C_j\, \e^{b_j t + \al(t) \log s} \left(1 + s_j \e^{-\i \pi \al(t)} \right)\ ,\qquad
	\al(t) = \al_0 + \al' t
}{el bsw R}
...
This model is a \abb{QCD}-inspired, eikonal-formulated model. Similarly to the previous models, the eikonal receives two contributions. One that controls high energy behavior (the authors call it even) and one responsible for the difference between $\rm pp$ and $\rm\bar pp$ scattering at low energies.
...
Eikonal (the leading one-half factor is not the paper, but is needed to get reasonable predictions)
\eqref{\ch'(b) = {1\over 2} \sum\limits_{j\in\lbrace \hbox{\SmallerFonts gg, qg, qq, odd} \rbrace} \si_j\, W(b, \mu_j)}{el bh ch}
Impact-parameter distribution
\eqref{W(b, \mu) = {\mu^2\over 96\pi} (\mu b)^3 K_3(\mu b)}{el bh W}
Sub-process cross-sections; SUM in Eq.~(B5) in \bref{bh99}
\eqref{\si_{\rm gg} = C'_{\rm gg} \Si_{\rm gg} N_{\rm g}^2 [SUM]}{el bh si gg}
\eqref{\si_{\rm qq} = \Si_{\rm gg} \left(C + C^{\rm even}_{\rm Regge} {m_0\over \sqrt s} \e^{\i{\pi\over 4}} \right)}{el bh si qq}
\eqref{\si_{\rm qg} = \Si_{\rm gg} C^{\rm log}_{\rm qg} \left( \log {s\over s_0} - \i {\pi\over 2} \right)}{el bh si qg}
\eqref{\si_{\rm odd} = -\i C_{\rm odd} \Si_{\rm gg} {m_0\over \sqrt s}  \e^{\i{\pi\over 4}} }{el bh si odd}

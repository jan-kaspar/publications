...
\> one can parameterize - how were the parameters calculated and their errors

\eqref{{\d N_{\rm B, el}\over \d t} = a \e^{b t + c t^2}}{felm bckg el t dist param}

\tab{felm bckg results}{\TODO{after acceptance correction = without acceptance requirement}}{
\omit&\multispan2\bhrulefill\cr
\omit&\omit\bvrule\bstrut\tskip 45 bottom -- 56 top\tskip&\hbox{45 top -- 56 bottom}\cr\bln
...
\bmfig
\fig{fig/pdf/felm_background_dist_t_el.pdf}{felm background dist t el}{[7cm]The background contamination in the elastic $t$-distribution from one diagonal. Before acceptance correction (blue) and after (red). The green curves represent fits according to parameterization \Eq{felm bckg el t dist param}.}
...
} are reconstructed by taking the average over the two one-arm measurements (the first terms in the square brackets of \Eq{felm xy reco} correspond to the left, the last terms to the right-arm measurements):

...
Taking the data from \Fg{felm background cmp} right, one can subtract the background (red) from signal+background (black), leading to an acceptance-corrected $t$-distribution. This distribution then enters the next step -- the unfolding.
...
. Then, after the acceptance correction etc., we would reconstruct a distribution $h'(\th')$ smeared due to the final angular resolution. These distributions can be related by an integral transformation (see e.g.~Sec.~16.6.1 in \bref{riley10} or Sec.~1.8.2 in \bref{becvar}):

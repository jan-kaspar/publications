...
\fig{fig/pdf/ttm_det_overview.pdf}{ttm det overview}{An overview of the TOTEM detectors. All detectors are installed symmetrically left (sector 45) and right (sector 56) of the interaction point 5 (IP5), only the \rhs~shown in the figure. Bottom part: the structure of the LHC around IP5. The vertical boxes represent quadrupole (Q) and dipole (D) magnets. D1 is the separation magnet -- splitting each beam to its proper beam-line, see the \rhs~of the figure. The RP stations at $147$ and $220\un{m}$ are shown by red dots (each dot represents a unit). Upper box: zoomed region (see the guiding dashed lines) around the IP5 -- the telescopes (red) T1 and T2 embedded into the CMS detector (gray).
}

\Fg{ttm det overview} shows an overview of the detector apparatus of TOTEM. symmetrically, all LV1 capable


Telescopes -- inelastic trigger, vertex reconstruction (background rejection)

\caption{Telescopes T1}

 (in the center of the CMS end-cap), $7.5$ to $10.5\un{m}$ from the IP, Sc.~5.2 in \bref{totem08}

\> $3.1 < \et < 4.7$

\> 1 telescope per side of IP5
...
\bmfig
\fig[,5cm]{fig/external/t1_quarter.jpg}{t1 quarter}{[7cm]T1 quarter}
\fig[,5cm]{fig/external/t1_installed.jpg}{t1 installed}{[7cm]T1 installation}
\emfig


\caption{Telescopes T2}

(after CMS HF calorimeter), centered around $13.5\un{m}$ from the IP, Sc.~5.3 in \bref{totem08}
...
\bmfig
\fig[,5cm]{fig/external/t2_quarter.jpg}{t2 quarter}{[7cm]T2 quarter}
\fig[,5cm]{fig/external/t2_installed.jpg}{t2 installed}{[7cm]T2 installation}
\emfig
...
\bmfig
\fig[,5cm]{fig/external/rp_station.jpg}{rp station}{[7cm]RP station}
\fig[,5cm]{fig/external/rp_unit.jpg}{rp unit}{[7cm]RP unit}
\emfig

\bmfig
\fig[,5cm]{fig/external/rp_pot.jpg}{rp pot}{[7cm]A Roman Pot}
\fig[,5cm]{fig/external/rp_package.jpg}{rp package}{[7cm]Detector package}
\emfig

\bmfig
\fig[,5cm]{fig/external/rp_hybrid.jpg}{rp hybrid}{[7cm]A hybrid with a silicon detector and four VFAT chips.}
\fig[,5cm]{fig/external/rp_blm_bpm.jpg}{rp sensor}{[7cm]BPM and BLM at a RP station.}
\emfig

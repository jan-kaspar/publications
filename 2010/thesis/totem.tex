...
The \abb{CSC} detectors are gaseous detectors (multi-wire proportional chambers) with both cathode plates segmented into strips. These strips and the anode wires are oriented at $60^\circ$ with respect to each other. Thus there are 3 read-out coordinates, which enables reconstruction of events with several 
...
The \abb{T2} telescopes are installed in the forward shielding of \abb{CMS} between the vacuum chamber and the inner shielding of the \abb{CMS} HF calorimeter, they are centered about $13.5\un{m}$ from the interaction point. The \abb{T2} telescopes cover the pseudorapidity region $5.3 < \et < 6.5$.

There is one telescope on each side of \abb{IP}5, see \Fg{t2 installed}. Each telescope consists of two quarters as shown in \Fg{t2 quarter}. Each quarter includes 5 semicircular planes, each equipped with 2 back-to-back mounted Gas Electron Multiplier (\abb{GEM}) detectors. Each detector has an azimuthal coverage of $192^\circ$, therefore there is an overlap between the two quarters within each telescope.

The \abb{GEM} detectors are gaseous detectors where the electron multiplication is realized by three perforated Cu-clad polyimide foils, separated by $2\un{mm}$. The inner radius of the perforation holes is $65\un{\mu m}$, their density lies between $50$ and $100/\un{mm^2}$ \TODO{check}. With an electric field about $3.6 \un{kV/cm}$ between the \abb{GEM} foils, the overall gas overall amplification is roughly $8000$. The \abb{GEM} detectors have double read-out: strips (radial coordinate) and pads (coarse radial and azimuthal). The strip radia range from $43$ to $144\un{mm}$, their width and spacing is $80$ and $400\un{\mu m}$ respectively. The pads, used for triggering, are of sizes from $2\times 2\un{mm^2}$ to $7\times 7\un{mm^2}$. The spatial resolution is about $0.15\un{mm}$ in the radial direction and $0.8^\circ$ in the azimuthal one. Further details can be found in Sec.~5.3 in \bref{totem08}.
...
In order to refer to any component of the \abb{RP} system a naming scheme was defined in \bref{totem-rp-naming}. Since we will use it in what follows, we summarize the scheme in \Tb{rp naming scheme}. For example, the 2nd plane in the near horizontal \abb{RP} in the $220\un{m}$ station in the right arm can be referred as 56-220-near-hor-2.
...
Since the \abb{RP}s are movable, each \abb{RP} module includes a linear voltage differential transformer (\abb{LVDT}), a device which can determine the approach of the \abb{RP} to the beam (for details see \bref{totem-lvdt}). These devices are calibrated to give the distance between the top/bottom face of the thin window (see \Fg{rp pot}) and the beam axis. If the gap between the thin window and the sensor's edges is known, one can calculate the distances from the beam to the sensor's cut edges (green lines in \Fg{rp station 3d}). These are the quantities relevant for the track reconstruction.
...
We will use the notation that quantities related to the \abb{IP} will be denoted with a star superscript, those without will be related to \abb{RP} stations. The energy of the protons is conserved during the \abb{LHC} transport from the \abb{IP} to \abb{RP} stations, therefore $p = p^*$ and $\xi = \si^*$. Thus we will not make distinction between their \abb{IP} and \abb{RP} values.
...
one has to add a term $D(s) \xi$ to the solution, see Sec.~5.2 in~\bref{wilson}. Then, for the proton transport from the \abb{IP} to a position $s$ one can write (see Sec.~5.2.4 in~\bref{wilson})
...
\Fg{ttm hit distribution} illustrates the properties of the three types of optics. The horizontal (vertical) spread of hits is determined by the horizontal effective length $L_x$ (the vertical $L_y$). That is why the $\be^* = 90$ and $2\un{m}$ optics yield very narrow elastic scattering hit distributions. The gray ellipses show $10\si$ contours, thereby controlling the positions of the \abb{RP}s.

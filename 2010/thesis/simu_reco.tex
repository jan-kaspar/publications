...
\> The aim of the {\bf pattern recognition} is to match sensor hits with particle tracks. This implicitly includes suppressing noise hits that are not associated with any track. There are two pattern recognition algorithms in the TOTEM software. Both of them search for linear patterns within the hits in each RP. The algorithm described in \Sc{pattern reco} can be used with tracks with arbitrary angles, the algorithm presented in Sc.~8.1.4 in \bref{hubert} is designed for the protons coming from the IP -- such protons are very parallel to the beam.
...
\TODO{angles -- important for alignment}
\TODO{problem with multiple tracks}
...
Our algorithm is an optimization of the Hough transform pattern search (see e.g.~Sc.2.4.3.4.~in\bref{bock}). Let us parameterize the line patterns in the following way ($q$ stands for either $u$ or $v$)

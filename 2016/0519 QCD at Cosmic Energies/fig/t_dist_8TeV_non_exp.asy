...
void DrawBand(RootObject o_c, RootObject o_r, pen p)
{
	guide gp, gm;
...
RootObject h_dsdt = RootGetObject(topDir+"DS-merged/"+f, base_dir + "/h_dsdt", error=false);
if (!h_dsdt.valid)
	write("ERROR: can't load fit data " + base_dir);

RootObject g_dsdt = RootGetObject(topDir+"DS-merged/"+f, base_dir + "/g_fit");

RootObject f_dsdt_fit = RootGetObject(topDir+"DS-merged/"+f, base_dir+"/fit canvas|f_fit");

RootObject h_rel_unc_anal = RootGetObject(topDir+"systematics/"+datasets_unc[0]+"/matrix_direct_"+diagonals[0]+".root",
	"all analysis/"+binning+"/h_stddev");

RootObject h_rel_unc_full = RootGetObject(topDir+"systematics/"+datasets_unc[0]+"/matrix_direct_"+diagonals[0]+".root",
...
void PlotOneFit(string f, string ds, string dgn, string binning, int n_b, string fit_type, string iteration, pen p)
{
	...
	RootObject ff = RootGetObject(f, base + "/fit canvas|f_fit", error=false);
	RootObject fd = RootGetObject(f, base + "/g_fit_data", error=false);
	...
	for (int dgni : diagonals.keys)
	{
		...
		*/
		...
		DrawRelDiffWithCorrection(
			RootGetObject(topDir+"DS-merged/fit.root", binning + "/"+datasets[dsi]+"/"+diagonals[dgni]+"/fit with systematics, n_b=1/fit canvas|h_dsdt"),
			RootGetObject(fu, "de_th_y/h_eff_syst"),

...
\newpage %-------------------------------------------------------------------------------------------
\title{TOTEM: forward hadronic phenomena at LHC}

\> LHC: $\approx 1/3$ of $\si_{\rm tot}$ flows more \Em{forward} than conventional experiments can detect

\> forward $\rightarrow$ \Em{low momentum transfer(s)} $\rightarrow$ \Em{non-perturbative QCD} $\rightarrow$ little explored/understood\\ ($\rightarrow$ interesting)

\> \Em{diffractive processes} $\Rightarrow$ non-suppressed \Em{rapidity gaps} $\Leftarrow$ \Em{exchange of colourless objects}

\vskip2mm

\centerline{\fig*[,8cm]{fig/ppInteraction.png}}
...
\newpage %-------------------------------------------------------------------------------------------
\title{(Some) processes of interest}

\vskip-3mm

\line{\fig*[5.8cm]{fig/ttm_diffractive_processes.pdf}\hskip3mm\vbox{\hsize9cm
\noindent$\leftarrow$ double line in diagrams: exchange of ``Pomeron'' -- a colourless object with vacuum quantum numbers
$$\Downarrow$$
\centerline{consequence: rapidity gap}

\vskip5cm

\line{\hss\hbox to5.8cm{\hss in contrast:\hss}}
\line{\hss\fig*[5.8cm]{fig/ttm_non_diffractive_process.pdf}}
...
\line{\AddBckg[0.2mm]{\fig[7cm]{fig/t1_installed.jpg}}\hskip3mm\raise15mm\vbox{\hsize7.5cm
\> installed inside CMS end-caps
\> at $7.5$ to $10.5\un{m}$ from the IP
\> one \cYe\em{telescope}\cFg{} on each side of IP
\> each telescope consists of two \cYe\em{quarters}\cFg{}
}\hss}

\vfil
\line{\AddBckg[0.2mm]{\fig[7cm]{fig/t1_quarter.jpg}}\hskip3mm\raise15mm\vbox{\hsize7.5cm
\> each quarter formed by 5 \cYe\em{planes\cFg} equally spaced along beam
\> each plane consists of 3 trapezoidal \em{\cYe CSC\break detectors\cFg}, each covering $60\un{^\circ}$ in azimuth
...
\> each quarter formed by 10 semi-circular\break \cYe\em{planes\cFg}, assembled in 5 back-to-back\break mounted pairs
\> each plane equipped with a \em{\cYe Gas Electron Multiplier\cFg} detector
...
\>> coarse radial$\times$azimuthal segmentation:\break \Em{pads} (for triggering, azimuthal resolution $0.8\un{^\circ}$)
}\hss}
...
\line{\AddBckg[0.2mm]{\fig[5cm]{fig/rp_station.pdf}}\hskip3mm\raise20mm\vbox{\hsize10cm
\> \cYe\em{stations}\cFg{} installed at $\pm 220\un{m}$ in the outgoing LHC beam-pipe
\> each station has two \cYe\em{units}\cFg{}, separated by $\approx 5\un{m}$
}\hss}

\line{\AddBckg[0.2mm]{\fig[5cm]{fig/rp_unit.pdf}}\hskip3mm\raise7mm\vbox{\hsize10cm
\> each unit contains 3 \cYe\em{Roman Pots}\cFg{}: top, bottom and horizontal
...
\line{\AddBckg[0.2mm]{\fig[5cm]{fig/rp_pot.pdf}}\hskip3mm\raise30mm\vbox{\hsize10cm
...
\line{\AddBckg[0.2mm]{\fig[6cm]{fig/rp_package.pdf}}\hskip3mm\raise39mm\vbox{\hsize10cm
\> each RP contains a \cYe\em{package}\cFg{} of 10 silicon sensors
\> 5 pairs of back-to-back mounted strip sensors
}\hss}

\vfil
\line{\AddBckg[0.2mm]{\fig[6cm]{fig/rp_hybrid.pdf}}\hskip3mm\raise23mm\vbox{\hsize10cm
...
\> \em{\cYe proton transport\cFg}: described as in linear optics
...
\noindent{\cYe\PartFont Theory\cFg}
...
\line{\hss various $\si_{\rm tot}$ fits by COMPETE\hss}
\line{\hss\fig*[11cm]{fig/ttm_sigma_tot.pdf}\hss}
...
\vskip-8mm
$$x_{\rm RP} = v_x x^* + L_x \th_x^* + \xi D_x\ ,\qquad \xi = {\De p / p_0}$$
\vskip+2mm
...
\> kinematics: {\BiggerFonts \cYe $\xi \approx e^{-\De \et}$\cFg}\\ $\Rightarrow$ double ``determination'' of $\xi$
...
\> $|t|$-distributions (unfolding not yet applied) fitted with \cLRe$\BiggerFonts \displaystyle \d\si / \d t = C\, \e^{-B t}$\cFg

%\vskip-1mm
\line{\hss\fig[,4.8cm]{fig/sd_t_dist_t2_opp.png}\hss\fig[,4.8cm]{fig/sd_t_dist_t1_opp.png}\hss}

% B of SD = B of ES / 2?

\> cross-section per class, including the invisible low-$|t|$ contribution (exploiting the fit above)

\line{\AddBckg[0.5mm]{\cBlack\tab{\bln
...
\centerline{\fig[10.5cm]{fig/DD_meas.png}}

\centerline{(T1: $3.1 < \et < 4.7$, T2: $5.3 < \et < 6.5$)}

\> comparison to Monte Carlos

\centerline{\fig[10.5cm]{fig/DD_mc_comp.png}}
...
\> TOTEM and CMS independent experiments -- \Em{common runs} need:
...
\vfil
\> TOTEM + CMS = \Em{unprecedented rapidity coverage}
\>> CMS tracker: $|η| < 2.5$
...
$\Rightarrow$ higher \Em{pile-up}

\> TOTEM: interest in lower-cross-section processes $\Rightarrow$ higher luminosity needed\\
$\Rightarrow$ higher \Em{pile-up}

\vfil
\noindent{\cYe Two aspects of coping with pile-up\cFg}

\> \Em{RP detectors resolve multiple tracks}

\>> two new units will be installed -- hosting \cGr pixel\cFg{} or \cGr rotated-strip\cFg{} detectors

\line{\hss\fig[12cm]{fig/multi_hit.pdf}\hss}

\> \Em{association of RP and central detector (CMS) tracks} $\rightarrow$ tracks with common vertex

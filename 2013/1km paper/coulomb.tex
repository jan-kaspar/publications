...
\item Section \ref{sec:fit exp3}: fits with nuclear modulus flexible enough to describe the non-exponentiality event without the CNI effects.
\end{itemize}
...
\begin{table}
\caption{Fit results with $N_b=1$. Each column corresponds to a different interference formula and/or nuclear phase. The peripheral phase parameters were allowed to vary within the ranges indicated in Eq.~(\ref{eq:nuc phase per val}).}
\vskip-3mm
\label{tab:fit exp1}
\begin{center}
...
\caption{Visualisation of fit results from Table~\ref{tab:fit exp1} obtained with KL formula and $N_b=1$.
...
The quality of the fit with peripheral phase is good, but \TODO{is it reasonable from other aspects?}
...
\begin{table}
\caption{Fit results with KL formula, $N_b=3$ and constant, Eq.~(\ref{eq:nuc phase con}), or peripheral, Eq.~(\ref{eq:nuc phase per}), nuclear phase. For the ``mid'' column, the peripheral phase parameters were free within the bounds in Eq.~(\ref{eq:nuc phase per val}), for the ``full'' they were fixed to the central values.}
\vskip-3mm
\label{tab:fit exp3}
\begin{center}
\setlength\tabcolsep{5mm}
...
\caption{Visualisation of fit results from Table~\ref{tab:fit exp3} obtained with KL formula and $N_b=3$.
TOP: fits compared to differential cross-section data in a relative reference frame, see the vertical axis label. The reference is identical to the one in \cite{8tev-90m}. 
BOTTOM LEFT: $t$-dependence of the nuclear phase as extracted from the fits.
BOTTOM RIGHT: the effects induced by the Coulomb interaction for each of the fits.
}%
...
\caption{Energy dependence of the $\rho$ parameter. The blue (green) hollow triangles correspond to $\rm pp$ ($\rm \bar pp$) data from PDG \cite{pdg}. The hollow dot stands for the earlier indirect determination by TOTEM \cite{epl101-tot}. The red triangles show the new results obtained in Section~\ref{sec:fit exp3}, using different nuclear phases. The black curve gives the preferred $\rm pp$ model by COMPETE \cite{compete}, obtained without using LHC data.
}%

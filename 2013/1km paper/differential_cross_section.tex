...
\todo{Short introduction - anticipate the argument flow; text below into a subsection}
...
The other acceptance correction, ${\cal A}_{\rm fluct}$, accounts for fluctuations around horizontal acceptance boundaries. \todo{How?} The uncertainties are related to the resolution parameters: vertical beam divergence size, left-right asymmetry and non-Gaussian shape.
...
The binning has been optimised with respect to two aspects. At low $|t|$, the bin size has been set to the triple of the resolution in $t$. At higher $|t|$, a fixed statistical fluctuation of $4\un{\%}$ has been targeted. \todo{define low and hight $|t|$}
...
\subsubsection{\todo{Cross checks}}

Compatible results have obtained from data in the beginning of data-taking periods (after beam cleanings) and in their ends (before next beam cleaning was needed). \todo{add diagonal, bunch, ... comparisons}
...
A summary and comparison of all systematic uncertainties is given in Fig.~\ref{fig:syst unc}. The leading ones are: normalisation uncertainty, optics imperfections, beam momentum offset and residual misalignment, their effect is quantified in Tab.~\ref{tab:data}. The vertical misalignment is the dominant systematic mode in the very low $|t|$ region. \todo{Give here the recipe to recover covariance matrix? This would leave more space on the table page.}
...
\caption{%
\todo{\todo{TODO: rename contributions to make them consistent among text, table and this figure; explain properly what needs to be abbreviated; then, shorten explications in the data table}. \todo{normalisation skipped as it is too big}}
}
\label{fig:syst unc}
\end{center}
\end{figure*}

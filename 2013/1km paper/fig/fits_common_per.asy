...
AddFit("peripheral", "KL", "per-ini-ran3", "1");	// [56.9,0.90|1.49,+3.58, 2.3,-0.29]
AddFit("parcmpper", "KL", "per-ini-ran3", "1");		//  [4.2,0.64|1.49,+3.57, 2.3,-0.29]

AddFit("peripheral", "KL", "per-ini-ran1", "2");	// [54.1,0.78|1.45,+4.56, 1.2,-0.38]
AddFit("parcmpper", "KL", "per-ini-ran3", "2");		//  [1.4,0.19|1.45,+4.31, 1.3,-0.36]

AddFit("peripheral", "KL", "per-ini-ran2", "3");	// [54.1,0.85|1.46,+3.11, 1.6,-0.27]
AddFit("parcmpper", "KL", "per-ini-ran3", "3");		//  [1.3,0.35|1.46,+3.22, 1.7,-0.26]
...
for (int dsi : datasets.keys)
{
	...
	*/
	...
	for (int fi : fits.keys)
	{
		...
		pen p = StdPen(fi + 1) + 1pt;
		p = StdPen(quotient(fi, 2) + 1) + 1pt;
		if (fi % 2 == 0) p += solid;
		if (fi % 2 == 1) p += dashed;

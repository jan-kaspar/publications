...
\TODO{add discussion on the source of non-exponentiality}
...
this analysis also exploits another, complementary data set with very strong statistical power~\cite{8tev-90m}, taken at the same energy but with different beam optics ($\beta^{*}=90\,$m) and covering a hadronically dominated $t$-range: 0.027\,GeV$^{2} < |t| < 0.2\,$GeV$^{2}$. \TODO{The isolated analysis of the
latter data set, neglecting effects from Coulomb scattering, has already excluded a purely exponential hadronic elastic scattering
...
form of the hadronic phase at small $|t|$ determines whether elastic collisions occur predominantly at small or large impact parameters (centrality vs.~peripherality). \TODO{Finalise: both are compatible with our data.}

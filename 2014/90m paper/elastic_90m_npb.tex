...
\title{Evidence for Non-Exponential Elastic Differential Cross-Section at Low $|\hbox{t}|$ and $\sqrt{\hbox{s}}$ = 8 TeV by TOTEM at the CERN LHC \todo{proton}}
...
$\sqrt s = 8\un{TeV}$ based on a new \todo{remove} high-statistics data sample obtained with 
the $\beta^* = 90\un{m}$ optics. 
Both statistical and systematic uncertainties remain below $1\un{\%}$, except for
the contribution from the overall normalisation \todo{which was obtained from 
another data set}. This unprecedented precision allows to exclude a purely exponential differential cross-section in the range of four-momentum transfer squared $0.027 < |t| < 0.2\un{GeV^2}$ with a significance greater than $7\un{\sigma}$. \todo{too big logical step} Consequently, the extrapolation to $t=0$ is reviewed, and two extended parameterisations are presented, yielding total cross-section estimates $(100.8 \pm 2.1)\un{mb}$ and $(101.2 \pm 2.1)\un{mb}$, well compatible with previous TOTEM measurements.
\end{abstract}

\begin{keyword}
\todo{proton} \sep elastic scattering \sep total cross-section \sep LHC \sep TOTEM

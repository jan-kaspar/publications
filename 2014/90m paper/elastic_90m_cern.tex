...
\> new: high-statistics data sample to complement the DS2 already presented in \cite{prl111}.
...
luminosity for each dataset, accounting for the data-acquisition (DAQ) inefficiency. The last column shows the lowest $|t|$ values observed. TODO: make compatible with PRL111 -- elastic events before acceptance correction? t min after all cuts = left edge of the left most bin?
}
\label{tab:datasets}
\begin{center}
\vskip-3mm
...
{\bf Normalisation}. TODO: ${\cal N}$, same integral between $|t| = 0.027$ and $0.083\un{GeV^2}$ wrt.~dataset 1 from \cite{prl111}

{\bf Binning} Up to $0.8\un{GeV^2}$ one smearing sigma.

{\bf TODO: Estimate of systematics} Acc. corr. due to non-gauss, L-R asymmetry, both few per-mille withing final data

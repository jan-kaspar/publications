...
The optics matching method \cite{totem-optics} has been applied. The residual uncertainty has a form of factors scaling the scattering angles: % double-arm; before matching $0.48\un{\%}$ (hor), $0.90\un{\%}$
...
Statistical fluctuations in the reconstructed scattering angles occur due to the beam divergence and, in horizontal projection, also sensor resolution (strip pitch). Good description of these effects is essential for several corrections discussed below.
...
Compatible results have obtained from data originating from different bunches and different diagonals. \todo{Shall try also different time periods as for 1000m?}
% and different time periods -- in particular those right after and right before the beam cleanings.
...
The plots quantify the impact of the systematic uncertainties on differential cross-section. The vertical axis gives the relative variation of differential cross-section.
}
\label{fig:syst unc}
\end{center}
\end{figure*}
...
The other columns are related to the differential cross-section. The right-most four columns give the leading systematic uncertainty modes (see Section \ref{sec:final data merging} and Figure \ref{fig:syst unc}). \todo{optimise number of digits, spacing}.
}
\label{tab:data}
\begin{center}
\small
...
\todo{describe fit quality measures? or in text?}
}
